...
public class UIInventory : MonoBehaviour
{
    ...
    public void OnUseButton()
    {
        if (selectedItem.item.type == ItemType.Consumable)
        {
            for (int i = 0; i < selectedItem.item.consumables.Length; i++)
            {
                ItemDataConsumable effect = selectedItem.item.consumables[i]; //  올바른 타입 사용
                ...
                switch (effect.type)
                {
                    ...
                    case ConsumableType.Jump:
                        StartCoroutine(ApplyJumpBoost(effect.value, 5f)); // 예제: 5초 지속
                        break;

                    case ConsumableType.speed:
                        StartCoroutine(ApplySpeedBoost(effect.value, 5f)); // 예제: 5초 지속
                        break;
                }
            }
